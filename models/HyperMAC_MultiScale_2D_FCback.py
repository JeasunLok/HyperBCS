...
class HyperMAC_2D_MultiScale_FCback(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv1(x)
        x_multiscale = self.conv1(x_multiscale)

        x = self.bn1(x)
        x_multiscale = self.bn1(x_multiscale)

        x = self.relu(x)
        x_multiscale = self.relu(x_multiscale)
...
if __name__ == '__main__':
    model = HyperMAC_2D_MultiScale_FCback().cuda()
